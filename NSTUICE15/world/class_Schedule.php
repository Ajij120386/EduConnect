...
$selected_batch = isset($_POST['selected_batch']) ? $_POST['selected_batch'] : "";
...
if (isset($_POST['submit_schedule'])) {
    foreach ($_POST['courses'] as $course) {
       ...
       $sql = "INSERT INTO class_update (date, course_name, start_time, end_time, venue, batch)
        VALUES ('$class_date', '$name', '$start', '$end', '$venue', '$selected_batch')";

        $conn->query($sql);
    }

    // ✅ Notify all users about the new schedule
    $users = mysqli_query($conn, "SELECT id FROM user_data");
    while ($row = mysqli_fetch_assoc($users)) {
        $uid = $row['id'];
        mysqli_query($conn, "
            INSERT INTO notifications (user_id, type, reference_id, message)
            VALUES ($uid, 'new_schedule', 0, 'A new class schedule has been shared.');
        ");
    }
...
<div class="container">
    ...
    <div class="card">
        ...
        <form method="post">
            ...
            <label><i class="fas fa-users-class"></i> Select Batch:</label>
<select name="selected_batch" required onchange="this.form.submit()">
    <option value="">--Select Batch--</option>
    <?php
    $batch_result = $conn->query("SELECT DISTINCT batch_no FROM user_data WHERE batch_no IS NOT NULL AND batch_no != '' ORDER BY batch_no ASC");
    while ($row = $batch_result->fetch_assoc()) {
        $b = $row['batch_no'];
        $is_selected = ($selected_batch == $b) ? 'selected' : '';
        echo "<option value='$b' $is_selected>$b</option>";
    }
    ?>
</select>
     ...
     <?php if ($selected_semester && $class_date && $selected_batch): ?>
