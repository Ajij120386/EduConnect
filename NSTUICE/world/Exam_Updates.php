...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['date'], $_POST['course'])) {
    ...
    foreach ($dates as $i => $exam_date) {
        ...
        if (!empty($exam_date) && !empty($course)) {
            $date = mysqli_real_escape_string($conn, $exam_date);

           $batch = mysqli_real_escape_string($conn, $_POST['batch']);
mysqli_query($conn, "INSERT INTO exam_update (course_name, date, batch) VALUES ('$course', '$date', '$batch')");

        }
    }

    echo "<script>alert('Exams Added Successfully!'); window.location.href='world.php';</script>";
    exit;
}
?>
